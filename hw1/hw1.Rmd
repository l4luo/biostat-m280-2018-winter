...
1. Apply for the [Student Developer Pack](https://education.github.com/pack) at GitHub using your UCLA email.  
    *I completed this step. My Github handle is @l4luo.*
...
2. Create a **private** repository `biostat-m280-2018-winter` and add `Hua-Zhou` and `juhkim111` as your collaborators with write permission.  
    *I created a private repo and added `Hua-Zhou` and `juhkim111` as collaborators.*
...
3. Top directories of the repository should be `hw1`, `hw2`, ... Maintain two branches `master` and `develop`. The `develop` branch will be your main playground, the place where you develop solution (code) to homework problems and write up report. The `master` branch will be your presentation area. Submit your homework files (R markdown file `Rmd`, `html` file converted from R markdown, all code and data sets to reproduce results) in `master` branch.  
    *I created a `master` and `develop` branch and created a `hw1` directory in each branch.*

4. After each homework due date, teaching assistant and instructor will check out your master branch for grading. Tag each of your homework submissions with tag names `hw1`, `hw2`, ... Tagging time will be used as your submission time. That means if you tag your `hw1` submission after deadline, penalty points will be deducted for late submission.  
    *I will tag the `hw` submission after merging the develop with the master branch.*
...
1. How many persons are in the data set (statisticians call this `n`)? How many SNPs are in the data set (statisticians call this `p`)?
    ...
    *The number of persons in this data set is:*
    ...
    ```
    *The number of SNPs in this dataset is:*
    ```{bash}
...
2. Which chromosomes does this data set contain? How many SNPs are in each chromosome?  
    *The second column of the output gives the chromosomes contained in the data set. The first column of the output gives the number of SNPs for each chromosome.*
    ```{bash}
...
4. Statistical geneticists often have to reformat a data set to feed into various analysis programs. For example, to use the Mendel software <http://www.genetics.ucla.edu/software/mendel>, we have to reformat the data set to be read by Mendel.
    ...
    ```
     ...
     - *I reformatted the plink `merge-geno.bim` data set to be readable by Mendel software, and then I piped the output to a new text file `mendelSNP`.*
    ```{bash}
    awk 'BEGIN {print "2.40 = FILE FORMAT VERSION NUMBER.\n"   \
    "8348674  = NUMBER OF SNPS LISTED HERE."} {OFS = ","} {print $2, $1, $4}'  \
    /home/m280-data/hw1/merge-geno.bim > mendelSNP
    head mendelSNP
    ```
    ...
    ```
    - *I converted the plink `merge-geno.fam` file to Mendel pedigree format, and then I piped the output to a text file `mendelPed`.*
    ```{bash}
    awk '{printf "%s,%s,%s,%s,%s,\n", $1, substr($2,5), $3==0 ? "" :   \
    substr($3,5), $4==0 ? "" : substr($4,5), $5==1 ? "M" : "F"}'   \
    /home/m280-data/hw1/merge-geno.fam > mendelPed 
    head -20 mendelPed
    ```
...
$$
for both methods.
    - *I expanded the `runSim.R` script and added a function (avgMSE) to generate data according to the specified arguments: `dist` for distribution type (gaussian, t1, or t5); `seed` for a random seed; `n` for sample size; `rep` for the number of simulation replicates; `whichMean` to specify which average estimator to compute (estMeanPrimes for the primed-indexed average estimator or mean for the classical sample average estimator).*
...
2. Modify the [`autoSim.R`](http://hua-zhou.github.io/teaching/biostatm280-2018winter/slides/02-linux/autoSim.R) script to run simulations with combinations of sample sizes `nVals = seq(100, 500, by=100)` and distributions `distTypes = c("gaussian", "t1", "t5")` and write output to appropriately named files. Use `rep = 50`, and `seed = 280`.   
    - *I modified the `autoSim.R` script to automatically set specified arguments for the `runSim.R` script. I then ran the Rscript in batch mode via the Linux terminal by calling the bash command below. 15 text files were outputted with the data generated from each run of `runSim.R`.*  
    ```{bash}
    Rscript autoSim.R
    ```

3. Write an R script to collect simulation results from output files and print average MSEs in a table of format  
    - *I wrote an Rscript `autoTable.R` that prints the average MSEs in a table.*  
    ```{bash, results = 'asis'}
    Rscript autoTable.R
    ```