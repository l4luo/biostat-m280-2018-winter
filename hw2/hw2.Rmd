...
1. What happens to missing values in a histogram? What happens to missing values in a bar chart? Why is there a difference?  
    *Missing values are removed in a histogram, along with a warning message: "Removed 2 rows containing non-finite values." In a bar chart, missing values...are also removed?? *
    ```{r}
    ...
    diamonds3 <- diamonds %>%
      mutate(color = ifelse(color == "I", NA, color))
    ggplot(data = diamonds3) +
...
1. Use what you’ve learned to improve the visualisation of the departure times of cancelled vs. non-cancelled flights.  
    *I used boxplots to compare the departure times of cancelled vs. non-cancelled flights. Cancelled flights should have either missing departure times or arrival times (from chapter 5 of "R For Data Science").*
    ```{r}
    flights2 <- flights %>%
...
6. If you have a small dataset, it’s sometimes useful to use `geom_jitter()` to see the relationship between a continuous and categorical variable. The ggbeeswarm package provides a number of methods similar to `geom_jitter()`. List them and briefly describe what each one does.  
    *`geom_quasirandom` jitters the points to reduce overplotting using the vipor package. In our case, it created a violin plot. `geom_beeswarm` jitters the points to reduce overplotting using the beeswarm package.*
    ```{r}
    ggplot(data = diamonds) +
      geom_jitter(aes(x = cut, y = price, color = cut)) 
    ggplot(data = diamonds) +
      geom_quasirandom(aes(x = cut, y = price, color = cut)) 
  #  ggplot(data = diamonds) +
  #    geom_beeswarm(aes(x = cut, y = price)) 
    ```
